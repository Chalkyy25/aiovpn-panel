    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="text" wire:model.defer="country_code" maxlength="2" class="form-input w-full uppercase" placeholder="GB">
  ...
  <x-section-card title="Monitoring & Maintenance" subtitle="Agent/health settings.">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div class="md:col-span-2">
        ...
        <input type="text" wire:model.defer="api_endpoint" class="form-input w-full" placeholder="https://node-agent:9000/">
      ...
      <div>
        ...
        <input type="text" wire:model.defer="api_token" class="form-input w-full" placeholder="secret…">
      ...
      <div class="md:col-span-2">
        ...
        <input type="text" wire:model.defer="health_check_cmd" class="form-input w-full" placeholder="systemctl is-active openvpn-server@server">
      ...
      <div>
        ...
        <input type="text" wire:model.defer="install_branch" class="form-input w-full" placeholder="stable">
        @error('install_branch') <p class="text-xs text-red-400 mt-1">{{ $message }}</p> @enderror
      </div>

      <div class="flex items-center gap-2">
        <input type="checkbox" id="monitoring_enabled" wire:model.defer="monitoring_enabled" class="h-4 w-4 rounded border-[var(--aio-border)]">
        <label for="monitoring_enabled" class="text-sm text-[var(--aio-ink)]">Monitoring Enabled</label>
      </div>
    </div>
  </x-section-card>
  ...
  <x-section-card title="Protocol Options" subtitle="OpenVPN / WireGuard tuning.">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div>
        ...
        <input type="text" wire:model.defer="ovpn_cipher" class="form-input w-full" placeholder="AES-256-GCM">
      </div>
      <div>
        <label class="block text-xs text-[var(--aio-sub)] mb-1">OpenVPN Compression</label>
        <input type="text" wire:model.defer="ovpn_compression" class="form-input w-full" placeholder="lz4-v2 / none">
      </div>
      ...
      <div class="md:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <textarea rows="2" wire:model.defer="wg_public_key" class="form-input w-full font-mono text-xs"></textarea>
        </div>
        <div>
          <label class="block text-xs text-[var(--aio-sub)] mb-1">WG Private Key</label>
          <textarea rows="2" wire:model.defer="wg_private_key" class="form-input w-full font-mono text-xs"></textarea>
        </div>
      </div>
    </div>
  </x-section-card>
  ...
  <x-section-card title="Limits & Policy" subtitle="Protect capacity & shape traffic.">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div>
        ...
        <input type="number" min="1" max="65000" wire:model.defer="max_clients" class="form-input w-full">
      ...
      <div>
        ...
        <input type="number" min="1" max="10000" wire:model.defer="rate_limit_mbps" class="form-input w-full">
        @error('rate_limit_mbps') <p class="text-xs text-red-400 mt-1">{{ $message }}</p> @enderror
      </div>
      <div class="flex items-center gap-2">
        <input type="checkbox" id="allow_split_tunnel" wire:model.defer="allow_split_tunnel" class="h-4 w-4 rounded border-[var(--aio-border)]">
        <label for="allow_split_tunnel" class="text-sm text-[var(--aio-ink)]">Allow Split Tunneling</label>
      </div>
    </div>
  </x-section-card>

  <x-section-card title="Notes">
    <textarea rows="3" wire:model.defer="notes" class="form-input w-full"></textarea>
  </x-section-card>
