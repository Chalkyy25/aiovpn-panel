...
@php
  ...
  $deltaStr = is_null($delta) ? null : (string) $delta;
  if (!is_null($deltaStr) && is_null($deltaUp)) {
    $deltaUp = !str_starts_with(trim($deltaStr), '-') && trim($deltaStr) !== '0';
  }

  $deltaIconRaw = $deltaUp ? 'o-arrow-up-right' : 'o-arrow-down-right';

  // ✅ Icon normaliser: allow o-*, s-* shorthand and auto-prefix to heroicon-*
  $normalizeIcon = function ($name) {
    ...
    if (str_starts_with($name, 'o-') || str_starts_with($name, 's-')) {
  ...
  $iconName      = $normalizeIcon($icon);
  $deltaIconName = $normalizeIcon($deltaIconRaw);
...
>
  ...
  @if(!is_null($deltaStr))
    <span class="inline-flex items-center gap-1 text-xs font-semibold {{ $deltaUp ? 'text-[var(--aio-neon)]' : 'text-[var(--aio-mag)]' }}">
      @if($deltaIconName)
        <x-icon :name="$deltaIconName" class="w-4 h-4" />
      @endif
      {{ $deltaStr }}
    </span>
  @elseif($hint)
