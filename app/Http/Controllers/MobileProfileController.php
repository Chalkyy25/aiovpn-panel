...
use App\Services\VpnConfigBuilder;   // ← add this
...
class MobileProfileController extends Controller
{
    public function index(Request $request)
    {
        ...
        return response()->json([
            ...
            'servers'   => $user->vpnServers()->get()->map(fn ($s) => [
                'id'   => $s->id,
    ...
    public function show(Request $request, VpnUser $user, VpnConfigBuilder $builder)
    {
        ...
        $server = $serverId
            ? $user->vpnServers()->where('id', $serverId)->first()
        ...
        try {
            ...
            // slight detail in error helps debugging if SSH/certs missing
            return response("Could not build config from {$server->ip_address}: ".$e->getMessage(), 502);
        }
