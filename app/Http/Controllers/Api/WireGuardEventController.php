...
class WireGuardEventController extends Controller
{
    ...
    public function store(Request $request, VpnServer $server): JsonResponse
    {
        ...
        DB::transaction(function () use ($server, $peers, $publicKeys, $now) {
            ...
            foreach ($peers as $p) {
                ...
                // Update peer stats too (optional but good)
                WireguardPeer::query()
                    ->where('vpn_server_id', $server->id)
        ...
        // Broadcast snapshot from DB (same event name, dashboard can merge/filter)
    ...
    private function disconnectMissingOrStale(int $serverId, array $touchedSessionKeys, Carbon $now): void
    {
        ...
        VpnConnection::query()
            ...
            ->where('is_active', 1)
            ->where(function ($q) use ($touchedSessionKeys, $cutoff) {
                if (!empty($touchedSessionKeys)) {
                    $q->whereNotIn('session_key', $touchedSessionKeys);
                }
                $q->orWhereNull('last_seen_at')
    ...
    private function parseTime(mixed $value): ?Carbon
    {
        ...
        $s = trim((string)$value);
        if ($s === '') return null;
        if (ctype_digit($s)) return $this->extractSeenAt(['handshake' => (int)$s]);
